...
public class PopulationUnderserved {
    ...
    /**
     * Gets the affected postal code by population
     * 
     * @param limit
     * @return
     */
    public List<String> getPostalCodeBasedOnPopulation(int limit) {
        Connection connect = null;
        Statement statement = null;
        List<String> damagedPostalCodesPopulationList = new ArrayList<>();
        ...
        try {
             ...
             */
            String stat = "SELECT DISTINCT postalCode FROM PostalCode order by population Desc limit " + limit + ";";
            resultSet = statement.executeQuery(stat);

            while (resultSet.next()) {
                damagedPostalCodesPopulationList.add(resultSet.getString("postalCode"));
            }
            statement.close();
            connect.close();
        } catch (Exception e) {
        ...
        return damagedPostalCodesPopulationList;
